...
/* Name: nbody_bad.c: parallel 2-d nbody simulation
 *
 * 2d torus.
 *
 * This has an error.  The acceleration in the y direction is computed
 * based on dx rather than dy.
 */
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
...
/* For i=1..nprocs-1: exchange by sendin i to your right while
  ...
  for (i=0; i<nprocs; i++) {
	...
	if (r_squared != 0) {
	  ...
	  if (r != 0) {
	    ...
	    /* This is an error: should be ay+= acceleration*dy/r */
	    ay += acceleration*dx/r;
	  }
	}
      }
